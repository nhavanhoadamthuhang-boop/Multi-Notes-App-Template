...
class NoteSortTest {
    ...
    fun sortNotes_newestFirst_ordersByCreatedAtDescending() {
        ...
        val sorted = when (NoteSortOrder.NEWEST_FIRST) {
            ...
            NoteSortOrder.MOST_COMMENTED -> list
        }
    ...
    @Test
    fun sortNotes_mostCommented_ordersByCommentCountDescending() {
        val note1 = NoteEntity(id = 1, title = "Note 1", description = "Desc 1")
        val note2 = NoteEntity(id = 2, title = "Note 2", description = "Desc 2")
        val note3 = NoteEntity(id = 3, title = "Note 3", description = "Desc 3")

        val commentsSummary = mapOf(
            1L to 5,  // Note 1 has 5 comments
            2L to 12, // Note 2 has 12 comments
            3L to 1   // Note 3 has 1 comment
        )

        val list = listOf(note1, note2, note3)
        val sorted = when (NoteSortOrder.MOST_COMMENTED) {
            NoteSortOrder.NEWEST_FIRST -> list.sortedByDescending { it.createdAt }
            NoteSortOrder.OLDEST_FIRST -> list.sortedBy { it.createdAt }
            NoteSortOrder.ALPHABETICAL -> list.sortedWith(compareBy(String.CASE_INSENSITIVE_ORDER) { it.title.trim() })
            NoteSortOrder.MOST_COMMENTED -> list.sortedByDescending { commentsSummary[it.id] ?: 0 }
        }

        assertEquals(listOf(note2, note1, note3), sorted)
