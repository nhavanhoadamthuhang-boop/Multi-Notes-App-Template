...
object DateUtils {
     ...
     * "[X] năm [X] tháng [X] tuần [X] ngày [X] giờ [X] phút [X] giây trước"
     * Ví dụ: 1 năm 1 tháng 0 tuần 0 ngày 0 giờ 0 phút 0 giây trước
     */
    fun formatDetailedElapsedTime(timestamp: Long): String {
        ...
        var totalDays = nowCal.get(Calendar.DAY_OF_MONTH) - thenCal.get(Calendar.DAY_OF_MONTH)
        ...
        if (totalDays < 0) {
            ...
            totalDays += prevMonthCal.getActualMaximum(Calendar.DAY_OF_MONTH)
            months--
        }
        if (months < 0) {
            months += 12
