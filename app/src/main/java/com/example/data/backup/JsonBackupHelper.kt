...
data class BackupComment(
    ...
    val avatarType: String = "INITIAL",
    val avatarValue: String? = null,
    val avatarBgColor: Long? = null,
...
object JsonBackupHelper {
    ...
    fun exportAllDataToJson(notes: List<NoteEntity>, comments: List<CommentEntity>): String {
        ...
        for (note in notes) {
            ...
            noteObj.put("isBookmarked", note.isBookmarked)
            ...
            if (note.imageUri != null) {
                noteObj.put("imageUri", note.imageUri)
            } else {
                noteObj.put("imageUri", JSONObject.NULL)
            }
            ...
            for (comment in noteComments) {
                ...
                commentObj.put("isBookmarked", comment.isBookmarked)
                commentObj.put("createdAt", comment.createdAt)
                commentObj.put("avatarType", comment.avatarType)
                if (comment.avatarValue != null) {
                    commentObj.put("avatarValue", comment.avatarValue)
                } else {
                    commentObj.put("avatarValue", JSONObject.NULL)
                }
                if (comment.avatarBgColor != null) {
                    commentObj.put("avatarBgColor", comment.avatarBgColor)
                } else {
                    commentObj.put("avatarBgColor", JSONObject.NULL)
                }
                if (comment.imageUri != null) {
                    commentObj.put("imageUri", comment.imageUri)
                } else {
                    commentObj.put("imageUri", JSONObject.NULL)
                }
    ...
    private fun parseSingleNoteObject(obj: JSONObject, externalCommentsArray: JSONArray? = null): BackupNote {
        ...
        val isBookmarked = obj.optBoolean("isBookmarked", false)
        ...
        val imageUri = if (obj.has("imageUri") && !obj.isNull("imageUri")) {
            obj.getString("imageUri").ifBlank { null }
        } else null
        ...
        if (commentsArray != null) {
            for (j in 0 until commentsArray.length()) {
                ...
                val isCommentBookmarked = commentObj.optBoolean("isBookmarked", false)
                val commentCreatedAt = commentObj.optLong("createdAt", System.currentTimeMillis())
                val avatarType = commentObj.optString("avatarType", "INITIAL").ifBlank { "INITIAL" }
                val avatarValue = if (commentObj.has("avatarValue") && !commentObj.isNull("avatarValue")) {
                    commentObj.getString("avatarValue").ifBlank { null }
                } else null
                val avatarBgColor = if (commentObj.has("avatarBgColor") && !commentObj.isNull("avatarBgColor")) {
                    commentObj.getLong("avatarBgColor")
                } else null
                val commentImageUri = if (commentObj.has("imageUri") && !commentObj.isNull("imageUri")) {
                    commentObj.getString("imageUri").ifBlank { null }
                } else null
