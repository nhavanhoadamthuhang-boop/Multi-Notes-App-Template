...
interface NoteDao {
    ...
    @Query("SELECT * FROM notes WHERE deletedAt IS NULL AND isArchived = 0 ORDER BY isPinned DESC, updatedAt DESC LIMIT 1")
    suspend fun getWidgetNote(): NoteEntity?
    ...
    suspend fun updateNoteBookmarked(id: Long, isBookmarked: Boolean)

    @Query("UPDATE notes SET imageUri = :imageUri, updatedAt = :updatedAt WHERE id = :id")
    suspend fun updateNoteImageUri(id: Long, imageUri: String?, updatedAt: Long = System.currentTimeMillis())
