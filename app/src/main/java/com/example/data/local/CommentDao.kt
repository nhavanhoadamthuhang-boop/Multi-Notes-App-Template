...
interface CommentDao {
    ...
    @Query("SELECT * FROM comments WHERE deletedAt IS NULL AND isArchived = 0 ORDER BY isPinned DESC, createdAt DESC LIMIT 1")
    suspend fun getWidgetComment(): CommentEntity?
    ...
    @Query("SELECT * FROM comments WHERE isArchived = 1 AND deletedAt IS NULL ORDER BY createdAt DESC")
    fun getArchivedComments(): Flow<List<CommentEntity>>
    ...
    @Query("UPDATE comments SET content = :content, authorName = :authorName, avatarType = :avatarType, avatarValue = :avatarValue, avatarBgColor = :avatarBgColor, imageUri = :imageUri WHERE id = :id")
    suspend fun updateCommentDetails(id: Long, content: String, authorName: String, avatarType: String, avatarValue: String?, avatarBgColor: Long?, imageUri: String?)

    @Query("UPDATE comments SET imageUri = :imageUri WHERE id = :id")
    ...
    @Query("UPDATE comments SET isBookmarked = :isBookmarked WHERE id = :id")
    suspend fun updateCommentBookmarked(id: Long, isBookmarked: Boolean)
